        ...
        <div class="container-fluid">
            <div class="col-md-12">
                <form action="/butcher/post" method="post">
                    ...
                    $butcher_id = Butcher::select('id')->orderBy('id', 'desc')->first();
                    if($butcher_id == null){
                        $butcher_id = 1;
                    } else {
                        $butcher_id = $butcher_id['id'] + 1;
                    }
                    ?>
                    <div id="dynamicTable">
                        <divv class="row">
                            ...
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" value="{{$butcher_id}}" name="detail[0][butcher_id]">
                                    <input type="text" class="form-control" placeholder="Input Price..." name="detail[0][price]" required>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $("#add").click(function() {
        ++i;
        $("#dynamicTable").append('<divv class="row"><div class="col-md-6"><div class="form-group"><select class="form-control" name="detail[' + i + '][grade_id]"><option selected="true" disabled="disabled">---Select Buthcers Grade---</option><option value="1" >Baik</option><option value="2">Lokal</option><option value="3">Afkir</option><option value="4">Jantan</option><option value="5">Kulit Kepala</option></select></div></div><div class="col-md-5"><div class="form-group"><input type="hidden" value="{{$butcher_id}}" name="detail[' + i + '][butcher_id]"><input type="text" class="form-control" placeholder="Input Price..." name="detail[' + i + '][price]"></div></div><div class="col-md-1 mt-3"><button type="button" class="btn btn-danger btn-sm remove-divv" ><i class="fas fa-minus"></i></button></div></divv>');
    });
